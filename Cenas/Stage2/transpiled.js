      ...
      value: function firstFrame() {
          ...
          [
            ...
            },
            {
              ...
              onReach: function onReach() {
                _this3.cloudFall.generationParams.images = [estrela];
                _this3.cloudFall.generationParams.scale = {
                  min: 0.2,
                  max: 1
                };
                _this3.cloudFall.generationParams.rotation = {
                  min: -15,
                  max: 15
                };
                _this3.cloudFall.generationParams.amount = 30;

                _this3.cloudFall.generateBehaviour = function (item, params) {
                  console.log("qwer");
                  item.scale =
                    (randomInt(5, 55) / 100) *
                    (params.scale.max - params.scale.min) +
                    params.scale.min;
                  item.rotation =
                    (randomInt(0, 100) / 100) *
                  ...
                  item.speed = {
                    x:
                      ((randomInt(0, 200) - 100) / 100) *
                      0.25 *
                      (0.9 * item.scale),
                    y: (randomInt(75, 100) / 100) * (0.9 * item.scale)
                  };
                  item.position = {
                    x: (randomInt(0, 100) / 100) * _this3.canvas.width,
        ...
        this.cloudFall = new ParticleFall({
          ...
          generateBehaviour: function generateBehaviour(item, params) {
            ...
            item.alpha =
              (randomInt(1, 45) / 100) * (1 - item.scale / params.scale.max) +
              0.01;
            item.speed = {
            ...
            item.position = {
              x: (randomInt(0, 100) / 100) * _this3.canvas.width,
              y: (_this3.canvas.height * randomInt(20, 80)) / 100
            };
            item.pivot = {
              x: 0.5,
              y: 0.5
        ...
        this.cloudFall.generateBehaviour = function (item, params) {
          item.scale =
            (randomInt(5, 55) / 100) * (params.scale.max - params.scale.min) +
            params.scale.min;
          item.rotation =
        ...
        this.ceu.mayRise = true; // =========================== Filminho inicial
...
var estrela = sheetLoader.queueSheet("../../img/stage1/Estrela.png"); // const plataforma = sheetLoader.queueSheet('../../img/stage1/Plataforma.png');
